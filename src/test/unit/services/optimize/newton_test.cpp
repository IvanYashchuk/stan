...
class ServicesOptimizeNewtonModelInterface : public testing::Test {
 public:
  ServicesOptimizeNewtonModelInterface()
      : init(init_ss), parameter(parameter_ss), model(2) {}

  std::stringstream init_ss, parameter_ss;
  stan::test::unit::instrumented_logger logger;
  stan::callbacks::stream_writer init;
  values parameter;
  stan::io::empty_var_context context;
...
TEST_F(ServicesOptimizeNewtonModelInterface, rosenbrock) {
  unsigned int seed = 0;
  unsigned int chain = 1;
  double init_radius = 0;
  ...
  ASSERT_EQ(3, parameter.names_.size());
  EXPECT_EQ("lp__", parameter.names_[0]);
  EXPECT_EQ("xy.1", parameter.names_[1]);
  EXPECT_EQ("xy.2", parameter.names_[2]);
...
TEST_F(ServicesOptimizeNewtonModelInterface, rosenbrock_no_save_iterations) {
  unsigned int seed = 0;
  unsigned int chain = 1;
  double init_radius = 0;
