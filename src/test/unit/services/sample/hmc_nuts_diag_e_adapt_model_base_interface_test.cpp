...
// this is test is copied from hmc_nuts_diag_e_adapt_test.cpp
// The code is exactly the same only model was changed to rosenbrock_model from
// test/unit/model/test_model_interface_rosenbrock.hpp

class ServicesSampleHmcNutsDiagEAdaptModelInterface : public testing::Test {
 public:
  ServicesSampleHmcNutsDiagEAdaptModelInterface() : model(2) {}

  std::stringstream model_log;
  stan::test::unit::instrumented_logger logger;
  stan::test::unit::instrumented_writer init, parameter, diagnostic;
  stan::io::empty_var_context context;
  rosenbrock_model model;
};

TEST_F(ServicesSampleHmcNutsDiagEAdaptModelInterface, call_count) {
...
TEST_F(ServicesSampleHmcNutsDiagEAdaptModelInterface, parameter_checks) {
  unsigned int random_seed = 0;
  unsigned int chain = 1;
  double init_radius = 0;
  int num_warmup = 200;
  ...
  EXPECT_EQ("xy.1", parameter_names[0][7]);
  EXPECT_EQ("xy.2", parameter_names[0][8]);
...
TEST_F(ServicesSampleHmcNutsDiagEAdaptModelInterface, output_sizes) {
  unsigned int random_seed = 0;
  unsigned int chain = 1;
  double init_radius = 0;
  int num_warmup = 200;
...
TEST_F(ServicesSampleHmcNutsDiagEAdaptModelInterface, output_regression) {
  unsigned int random_seed = 0;
  unsigned int chain = 1;
  double init_radius = 0;
  int num_warmup = 200;
